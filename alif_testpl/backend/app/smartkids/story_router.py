...
def _language_to_speech_settings(language: str) -> Dict[str, str]:
    if language == "ru-RU":
...
async def next_question(request: NextQuestionRequest):
    ...
    try:
        ...
        request.language = detect_language_code(request.story_text)
        print(f"🌍 Lang detected -> {request.language}")
...
async def detect_language_endpoint(request: DetectLanguageRequest):
    """Frontend uchun: story_text bo'yicha tilni aniqlab, TTS/STT sozlamalarini qaytaradi."""
    language = detect_language_code(request.text)
    return _language_to_speech_settings(language)
