...
# ============================================================
# NEW ORGANIZATION MODELS (Subscription, Material, Schedule)
# ============================================================
...
class OrganizationSubscription(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    organization_id = Column(UUID(as_uuid=True), ForeignKey("organization_profiles.id", ondelete="CASCADE"), nullable=False)
    
    plan_type = Column(String(50), default="standard") # standard, premium, enterprise
    start_date = Column(DateTime(timezone=True), default=func.now())
    end_date = Column(DateTime(timezone=True), nullable=False)
    ...
    organization = relationship("OrganizationProfile", backref="subscription_details")

class OrganizationMaterial(Base):
    """
    Content Box for Organization (Books, Manuals, Tests)
    """
    __tablename__ = "organization_materials"
    ...
    file_type = Column(String(50), nullable=True) # pdf, doc, video
    category = Column(String(100), nullable=True) # handbook, test, textbook
    
    created_by_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    
    organization = relationship("OrganizationProfile", backref="materials")
    created_by = relationship("User")

class ClassroomSchedule(Base):
    """
    Weekly Schedule for Classrooms
    """
